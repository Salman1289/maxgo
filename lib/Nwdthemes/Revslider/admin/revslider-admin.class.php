...
class RevSliderAdmin extends RevSliderBaseAdmin{
	...
	public static function enqueue_all_admin_scripts() {
        ...
        foreach($post_types as $post_type) {
            if($post_type == $screen->id){
                Mage::helper('nwdrevslider/framework')->wp_enqueue_script('revslider-tinymce-shortcode-script', RS_PLUGIN_URL . 'admin/assets/js/tinymce-shortcode-script.js', array('jquery'), RevSliderGlobals::SLIDER_REVISION );
            }
        }
	}
	...
	protected static function get_javascript_multilanguage(){
		$lang = array(
			...
			'start_video' => __('Start Video'),
			'stop_video' => __('Stop Video'),
            'mute_video' => __('Mute Video'),
            'unmute_video' => __('Unmute Video'),
			'toggle_layer_anim' => __('Toggle Layer Animation'),
			'toggle_video' => __('Toggle Video'),
            'toggle_mute_video' => __('Toggle Mute Video'),
		...
		if($uid == ''){
			...
			if($filepath !== false && !is_array($filepath)){
				...
				if(is_array($filepath)){
					...
					Mage::helper('nwdrevslider/framework')->dmp(__("Could not download from server. Please try again later!"));
				}
				echo RevSliderFunctions::getHtmlLink($viewBack, __("Go Back"));
				exit;
			}
		...
		try{
			...
			switch($action){
				...
				case "replace_image_urls":
					...
					self::ajaxResponseSuccess(__("Image urls replaced"));
				break;
				...
				case "activate_purchase_code":
                    ...
                    if(!empty($data['code'])){
						...
						RevSliderFunctions::throwError(__('The API key, the Purchase Code and the Username need to be set!'));
						exit();
					}
