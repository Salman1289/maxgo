...
class RevSliderTinyBox {
    ...
    public static function add_tinymce_editor(){
        ...
        $post_types = Mage::helper('nwdrevslider/framework')->get_post_types();
        ...
        <div id="revslider-tiny-mce-dialog" tabindex="-1" action="" title="" style="display: none; ">
            <form id="revslider-tiny-mce-settings-form" action="">
                ...
                <div id="revslider-tiny-dialog-step-1">
                    ...
                    <div class="revslider-quick-inner-wrapper" style="padding-right:0px;padding-bottom:0px;">
                        <select name="revslider-existing-slider" id="revslider-existing-slider">
                            <option value="-1" selected="selected"><?php echo Mage::helper('nwdrevslider')->__('--- Choose Slider ---'); ?></option>
                            <?php
                            $sl = array();
                            $sliders_info = array();
                            if(!empty($sliders)){
                                ...
                                if(!empty($sl)){
                                    foreach($sl as $type => $slider){
                                        ...
                                        echo '<option disabled="disabled">--- '.ucfirst(Mage::helper('nwdrevslider/framework')->esc_attr($mtype)).' ---</option>';
                                        foreach($slider as $values){
                                            if($values['alias'] != 'false'){
                                                echo '<option data-sliderid="'.Mage::helper('nwdrevslider/framework')->esc_attr($values['id']).'" data-slidertype="'.Mage::helper('nwdrevslider/framework')->esc_attr($type).'" value="'.Mage::helper('nwdrevslider/framework')->esc_attr($values['alias']).'">'.Mage::helper('nwdrevslider/framework')->esc_attr($values['title']).'</option>'."\n";
                                            }
                                        }
                                    }
                                }
                        ...
                        <ul id="rs-shortcode-select-wrapper">
                            <li class="rs-slider-modify-li rs-slider-modify-new-slider">
                                <a href="<?php echo RevSliderBaseAdmin::getViewUrl(RevSliderAdmin::VIEW_SLIDER); ?>" target="_blank" class="button-primary revgreen" id="rs-create-predefined-slider"><span class="dashicons dashicons-plus"></span><span class="rs-slider-modify-title"><?php echo Mage::helper('nwdrevslider')->__('Create Slider'); ?></span></a>
                            </li>
                            <?php
                            if(!empty($sliders_info)){
                                foreach($sliders_info as $type => $slider){
                                    foreach($slider as $values){
                                        ?>
                                        <li id="slider_list_item_<?php echo $values['sliderid']; ?>" class="rs-slider-modify-li" data-sliderid="<?php echo Mage::helper('nwdrevslider/framework')->esc_attr($values['sliderid']); ?>" data-slideralias="<?php echo Mage::helper('nwdrevslider/framework')->esc_attr($values['slideralias']); ?>">
                    ...
                    <div class="revslider-stepnavigator">
                        <span class="revslider-currentstep"><strong><?php echo Mage::helper('nwdrevslider')->__('STEP 1'); ?></strong><?php echo Mage::helper('nwdrevslider')->__('Select / Create Slider'); ?></span>
                        <span class="revslider-step-actions-wrapper">
                            <a href="javascript:void(0);" class="button-primary nonclickable" id="rs-modify-predefined-slider"><span class="dashicons dashicons-admin-generic"></span><?php echo Mage::helper('nwdrevslider')->__('Quick Modify Slider'); ?></a>
                            <a href="javascript:void(0);" class="button-primary nonclickable" id="rs-add-predefined-slider"><span class="dashicons dashicons-migrate"></span><?php echo Mage::helper('nwdrevslider')->__('Add Selected Slider'); ?></a>
                        </span>
                        <span style="clear:both;width:100%;display:block"></span>
                    </div>
                </div>
                ...
                <div id="revslider-tiny-dialog-step-1-5" style="display: none;">
                    ...
                    <div class="revslider-stepnavigator"><span class="revslider-currentstep"><strong><?php echo Mage::helper('nwdrevslider')->__('STEP 2'); ?></strong><?php echo Mage::helper('nwdrevslider')->__('Quick Modify / Add Slider'); ?></span>
                        <span class="revslider-step-actions-wrapper">
                            <a href="javascript:void(0);" class="button-primary  rs-goto-step-1"><span class="dashicons dashicons-arrow-left-alt"></span><?php echo Mage::helper('nwdrevslider')->__('Select Slider'); ?></a>
                            <a href="javascript:void(0);" class="button-primary " id="revslider-add-custom-shortcode-modify"><span class="dashicons dashicons-migrate"></span><?php echo Mage::helper('nwdrevslider')->__('Add Selected Slider'); ?></a>
                        </span>
                    </div>
                </div>
            </form>
