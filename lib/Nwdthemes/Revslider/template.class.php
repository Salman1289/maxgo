...
class RevSliderTemplate {
				...
				if($response = $request['body']) {
					if($response !== 'invalid'){
						...
						if($ret !== false){
							...
							return array('error' => __('Can\'t write the file into the folder folder "' . $upload_dir['basedir'].$this->templates_path . '", please change permissions and try again!'));
						}
					}
				}
			}//else, check for error and print it to customer
		}else{
			return array('error' => __('Can\'t write into the folder "' . $upload_dir['basedir'].$this->templates_path . '", please change permissions and try again!'));
		}
			...
			if(!Mage::helper('nwdrevslider/framework')->is_wp_error($request)) {
				if($response = Mage::helper('nwdrevslider/framework')->maybe_unserialize($request['body'])) {
					...
					if(is_array($templates)) {
						Mage::helper('nwdrevslider/framework')->update_option('rs-templates-new', $templates);
					}
				}
			}
	...
	private function update_template_list(){
		...
		if($new !== false && !empty($new) && is_array($new)){
			...
			Mage::helper('nwdrevslider/framework')->update_option('rs-templates', $cur);
			Mage::helper('nwdrevslider/framework')->update_option('rs-templates-new', false);
	...
	private function _update_images(){
		...
		Mage::helper('nwdrevslider/framework')->update_option('rs-templates', $templates); //remove the push_image
	}
	...
	public function write_import_template_markup($template){
		...
		<div data-src="<?php echo $template['img']; ?>" class="template_slider_item_import<?php echo $deny; ?>"
            ...
            if($deny == ''){
                ...
                <div class="rs-required-to-dl">
                    <?php echo Mage::helper('nwdrevslider')->__('V'); ?> <?php echo $template['required']; ?>+
                </div>
                <?php
            }
            ?>
		...
		<div style="position:absolute;top:10px;right:50px;width:35px;text-align:right;z-index:2">
			...
			<a class="icon-preview_slider" href="<?php echo Mage::helper('nwdrevslider/framework')->esc_attr($template['preview']); ?>" target="_blank"></a>
			<?php } ?>
		</div>
		<?php
	}
