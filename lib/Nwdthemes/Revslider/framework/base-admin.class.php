...
class RevSliderBaseAdmin extends RevSliderBase {
	...
	public function __construct($t){
		...
		Mage::helper('nwdrevslider/framework')->add_action('admin_menu', array('RevSliderBaseAdmin', 'addAdminMenu'));
		Mage::helper('nwdrevslider/framework')->add_action('add_meta_boxes', array('RevSliderBaseAdmin', 'onAddMetaboxes'));
		Mage::helper('nwdrevslider/framework')->add_action('save_post', array('RevSliderBaseAdmin', 'onSavePost'));
		
		//if not inside plugin don't continue
		if($this->isInsidePlugin() == true){
			Mage::helper('nwdrevslider/framework')->add_action('admin_enqueue_scripts', array('RevSliderBaseAdmin', 'addCommonScripts'));
			Mage::helper('nwdrevslider/framework')->add_action('admin_enqueue_scripts', array('RevSliderAdmin', 'onAddScripts'));
        }else{
            Mage::helper('nwdrevslider/framework')->add_action('admin_enqueue_scripts', array('RevSliderBaseAdmin', 'addGlobalScripts'));
		}
	...
	protected static function requireView($view){
		try{
			...
			if(!empty(self::$master_view) && !isset(self::$tempVars["is_masterView"]) ){
				...
				if(!in_array($view, self::$allowed_views)) UniteFunctionsRev::throwError(__('Wrong Request'));
	...
	public static function addAdminMenu(){
		...
		foreach(self::$arrMenuPages as $menu){
            ...
            $revslider_screens[] = add_menu_page( $title, $title, $role, array(self::$t, $pageFunctionName), 'dashicons-update' );
		}
