<?php
class Olegnax_Athlete_Block_Product_List_Bestsellers extends Mage_Catalog_Block_Product_List
{
	...
	protected function _construct()
	{
		$this->addData(array(
			...
			'cache_tags'     => array('olegnax_athlete_product_list_bestsellers'),
		));
	}
	...
	public function getCacheKeyInfo()
	{
		if (NULL === $this->_cacheKeyArray)
		{
			$this->_cacheKeyArray = array(
				...
				$this->getCategoryId(), // GP - added category filter
	...
	protected function _getProductCollection()
	{
		...
		// GP - added category filter
		if ( $this->getCategoryId() ) {
			$category = Mage::getModel('catalog/category')->load($this->getCategoryId());
			$products->addCategoryFilter($category);
		}
		// END GP - added category filter
