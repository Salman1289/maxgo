 ...
 */
class Olegnax_Colorswatches_Helper_Productlist extends Mage_Core_Helper_Abstract
{
    /**
    ...
    {
        if (Mage::helper('olegnaxcolorswatches')->isEnabled()
            ...
            if ($layer = $block->getLayer()) {
                foreach ($layer->getFilterableAttributes() as $attribute) {
                    if (Mage::helper('olegnaxcolorswatches')->attrIsSwatchType($attribute)) {
                        $block->getChild($attribute->getAttributeCode() . '_filter')
                            ->setTemplate('olegnax/colorswatches/catalog/layer/filter/swatches.phtml');
                    }
                }
            }
            ...
            if (false && $stateRenderersBlock = $block->getChild('state_renderers')) {
                $swatchRenderer = Mage::app()->getLayout()
                    ->addBlock('olegnaxcolorswatches/catalog_layer_state_swatch', 'product_list.swatches');
                $swatchRenderer->setTemplate('olegnax/colorswatches/catalog/layer/state/swatch.phtml');
    ...
    public function getSwatchAttributeId()
    {
        if (is_null($this->_swatchAttributeId)) {
            $this->_swatchAttributeId =
                Mage::getStoreConfig(Olegnax_Colorswatches_Helper_Data::CONFIG_PATH_LIST_SWATCH_ATTRIBUTE);
        }
