...
class RevSliderSlide extends RevSliderElementsBase{
	...
	public function initByData($record){
		...
		//Mage::helper('nwdrevslider/framework')->dmp($imageUrl);exit();
	...
	public function set_post_data($text, $attr, $post_id){
		...
		if(RevSliderWooCommerce::isWooCommerceExists()){
			...
			if(strpos($text, 'wc_add_to_cart_button') !== false){
				...
				$ajax_cart_en			= Mage::helper('nwdrevslider/framework')->get_option( 'woocommerce_enable_ajax_add_to_cart' ) == 'yes' ? true : false;
				$assets_path          	= str_replace( array( 'http:', 'https:' ), '', WC()->plugin_url() ) . '/assets/';
				$frontend_script_path 	= $assets_path . 'js/frontend/';
				
				if ( $ajax_cart_en ){
					Mage::helper('nwdrevslider/framework')->wp_enqueue_script( 'wc-add-to-cart', $frontend_script_path . 'add-to-cart' . $suffix . '.js', array( 'jquery' ), WC_VERSION, true );
					
					global $wc_is_localized;
					if($wc_is_localized === false){ //load it only one time
						Mage::helper('nwdrevslider/framework')->wp_localize_script( 'wc-add-to-cart', 'wc_add_to_cart_params', Mage::helper('nwdrevslider/framework')->apply_filters( 'wc_add_to_cart_params', array(
							'ajax_url'                => WC()->ajax_url(),
							'ajax_loader_url'         => Mage::helper('nwdrevslider/framework')->apply_filters( 'woocommerce_ajax_loader_url', $assets_path . 'images/ajax-loader@2x.gif' ),
							'i18n_view_cart'          => esc_attr__( 'View Cart', 'woocommerce' ),
							'cart_url'                => Mage::helper('nwdrevslider/framework')->get_permalink( wc_get_page_id( 'cart' ) ),
							'is_cart'                 => is_cart(),
							'cart_redirect_after_add' => Mage::helper('nwdrevslider/framework')->get_option( 'woocommerce_cart_redirect_after_add' )
						) ) );
						$wc_is_localized = true;
					}
				}
				$wc_add_to_cart_button = Mage::helper('nwdrevslider/framework')->apply_filters( 'woocommerce_loop_add_to_cart_link',
									sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" class="button %s product_type_%s">%s</a>',
										Mage::helper('nwdrevslider/framework')->esc_url( $product->add_to_cart_url() ),
										Mage::helper('nwdrevslider/framework')->esc_attr( $product->id ),
										Mage::helper('nwdrevslider/framework')->esc_attr( $product->get_sku() ),
										$product->is_purchasable() ? 'add_to_cart_button' : '',
										Mage::helper('nwdrevslider/framework')->esc_attr( $product->product_type ),
	...
	private function updateLayersImages($layers = array()) {
		foreach ($layers as $key => $layer) {
			if (isset($layer['image_url']) && ! empty($layer['image_url'])) {
				$layers[$key]['image_url'] = Mage::helper('nwdrevslider/images')->image_to_url($layer['image_url']);
			}
		}
		return $layers;
	}
