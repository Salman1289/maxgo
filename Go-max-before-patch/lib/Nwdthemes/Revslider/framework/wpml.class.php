...
class RevSliderWpml{
	...
	public static function getArrLanguages($getAllCode = true){
		...
		/* NEW:
		$arrLangs = Mage::helper('nwdrevslider/framework')->apply_filters( 'wpml_active_languages', array() );
		*/
	...
	public static function getFlagUrl($code){
		...
		if(empty($code) || $code == "all")
			$url = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'admin/assets/images/icon-all.png';
		else
		...
		if ( empty( $code ) || $code == "all" ) {
            ...
            $active_languages = Mage::helper('nwdrevslider/framework')->apply_filters( 'wpml_active_languages', array() );
	...
	public static function getLangTitle($code){
		...
		return("");
		/* NEW: 
		$default_language = Mage::helper('nwdrevslider/framework')->apply_filters( 'wpml_default_language', null );
        return Mage::helper('nwdrevslider/framework')->apply_filters( 'wpml_translated_language_name', '', $code, $default_language );
		*/
	}
	...
	public static function getCurrentLang(){
		...
		/* NEW:
		if ( Mage::helper('nwdrevslider/framework')->is_admin() ) {
            return Mage::helper('nwdrevslider/framework')->apply_filters( 'wpml_default_language', null );
        }
        return Mage::helper('nwdrevslider/framework')->apply_filters( 'wpml_current_language', null );
		*/
