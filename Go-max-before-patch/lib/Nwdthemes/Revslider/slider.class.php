...
class RevSliderSlider extends RevSliderElementsBase{
	...
	public function initByAlias($alias){
		...
		try{
			...
			if(!empty($arrAliases)){
                ...
                $strAliases = "'".implode("' or '", Mage::helper('nwdrevslider/framework')->sanitize_text_field($arrAliases))."'";
			}

			$errorMessage = 'Slider with alias <strong>'.Mage::helper('nwdrevslider/framework')->esc_attr($alias).'</strong> not found.';
			if(!empty($strAliases))
	...
	private function duplicateSlider($title = false){
		...
		if($title === false){
			...
			$newSliderAlias = Mage::helper('nwdrevslider/framework')->sanitize_title($title);
		...
		try{
            ...
            if( !Mage::helper('nwdrevslider/framework')->is_wp_error($unzipfile) ){
				...
				if(!empty($animations)){
					...
					Mage::helper('nwdrevslider/framework')->dmp(__("no custom animations found, if slider uses custom animations, the provided export may be broken..."));
				}
				...
				if(is_array($dynamicCss) && $dynamicCss !== false && count($dynamicCss) > 0){
					...
					Mage::helper('nwdrevslider/framework')->dmp(__("no dynamic styles found, if slider uses dynamic styles, the provided export may be broken..."));
				}
    ...
    public function getSlidesForOutput($publishedOnly = false, $storeID = 0, $lang = 'all',$gal_ids = array()){
		...
		if(RevSliderWpml::isWpmlExists() && $this->getParam('use_wpml', 'off') == 'on'){
			...
			/* NEW: 
			$wpml_current_language = Mage::helper('nwdrevslider/framework')->apply_filters( 'wpml_current_language', null );
			Mage::helper('nwdrevslider/framework')->do_action( 'wpml_switch_language', $lang );
			*/
		}
		...
		if(RevSliderWpml::isWpmlExists() && $this->getParam('use_wpml', 'off') == 'on'){ //switch language back
			...
			// NEW: Mage::helper('nwdrevslider/framework')->do_action( 'wpml_switch_language', $wpml_current_language );
		}
