...
class Olegnax_Athlete_Helper_Image extends Mage_Core_Helper_Abstract
{
	...
	public function getAdditionalImage($product, $imgX, $imgY, $listing = false)
	{
		...
		if ($img) {
			return '<img' . ($listing ? ' id="product-collection-image-'.$product->getId().'" ' : '') .' class="additional_img" src="' . Mage::helper('catalog/image')->init($product, 'small_image', $img->getFile())->resize($imgX, $imgY) .
