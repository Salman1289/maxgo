...
class Olegnax_Athlete_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function getConfigurableJs($product)
	{ 
		if(Mage::helper('core')->isModuleEnabled('Mage_ConfigurableSwatches') && Mage::helper('core')->isModuleOutputEnabled('Mage_ConfigurableSwatches') && Mage::getStoreConfig('configswatches/general/enabled') && $product->hasOptions())
		{
			return $this->getLayout()->createBlock('Olegnax_Athlete_Block_Configurable_Catalog_Media_Js_Product', 'configurable_catalog_media_js_product')->setTemplate('configurableswatches/catalog/media/quickviewjs.phtml')->toHtml();
		}
		return;
	}
